...
function _mergeFiles(base, proposed, override) {
  ...
  const byHash = base.reduce((result, item) => {
    Object.keys(item.hashes).reduce(hashKey => {
      result[hashKey] = result[hashKey] || {}
      result[hashKey][item.hashes[hashKey]] = result[hashKey][item.hashes[hashKey]] || []
      result[hashKey][item.hashes[hashKey]].push(item)
    })
    return result
  }, {})
  proposed.forEach(file => {
    ...
    const hashesToMatch = Object.assign(file.hashes, entry ? entry.hashes : null)
    const matching = Object.keys(hashesToMatch)
      .reduce((result, key) => {
        const keyHashes = byHash[key]
        const addenda = keyHashes ? keyHashes[hashesToMatch[key]] : null
        if (addenda) result.push(...addenda)
        return result
      }, [])
      .filter((val, idx, arr) => {
        return val !== entry && idx === arr.indexOf(val)
      })
    matching.forEach(v => _mergeFile(v, file, override))
  })
  return base
}
